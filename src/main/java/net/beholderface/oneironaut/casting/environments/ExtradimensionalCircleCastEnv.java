    ...
    public ExtradimensionalCircleCastEnv(CircleCastEnv parent, ServerLevel target, @Nullable CastingVM existingVM,
        ...
        if (visitedLoci.size() != 8){
            ...
            for (BlockPos pos : lociList){
                AABB testBox = new AABB(Vec3.atLowerCornerOf(corner1), Vec3.atLowerCornerOf(pos));
            ...
            if (validBox){
                double scaleMultiplier = this.parentEnv.getWorld().dimensionType().coordinateScale()
                        / this.world.dimensionType().coordinateScale();
