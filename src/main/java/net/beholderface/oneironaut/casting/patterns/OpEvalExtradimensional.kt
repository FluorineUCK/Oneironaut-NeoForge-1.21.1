...
class OpEvalExtradimensional : Action {
    override fun operate(env: CastingEnvironment, image: CastingImage, continuation: SpellContinuation
        ...
        val copiedImage = image.copy(stack = TreeList.from(stack))
        // The pre-39 VM calls operate() while probing the next action, then discards
        // its image and side effects. This action performs a nested VM evaluation
        // directly, so short-circuit it to keep Simulate free of world effects.
        if (image.simulateNext) {
            return OperationResult(copiedImage.withUsedOp(), listOf(), continuation, HexEvalSounds.HERMES.get())
        }
    ...
    fun execPlayer(env: ExtradimensionalCastEnv, image: CastingImage, continuation: SpellContinuation, instrs: Iota): OperationResult {
        ...
        return OperationResult(subHarness.image.withUsedOp(), listOf(), continuation, HexEvalSounds.HERMES.get())
    }
