...
class OpDimTeleport : SpellAction {
    ...
    override fun execute(args: List<Iota>, env: CastingEnvironment): SpellAction.Result {
        val target = args.getNonlivingIfAllowed(env.world, 0, argc)
        ...
        val isImmune = !target.canUsePortal(false) || target.type.`is`(HexTags.Entities.CANNOT_TELEPORT)
        val isInCircleBounds = env is CircleCastEnv && env.circleState().bounds.aabb().containsPermissive(target.position())
        val teleportingOtherPlayer = target.isAlwaysTicking && target != env.castingEntity && !OneironautConfig.server.planeShiftOtherPlayers
    ...
    private data class Spell(var target: Entity, val origin: ServerLevel, val destination: ServerLevel, val coords: Vec3, val noosphere: Boolean) : RenderedSpell {
        override fun cast(env: CastingEnvironment) {
            ...
            if (!(destination.getBlockState(scanPoint).block.equals(Blocks.BEDROCK))){
                if (isUnsafe(destination, BlockPos(Vec3(x, (scanPoint.y - 1).toDouble(), z).toVec3i()), true) || !isSolid(destination, BlockPos(Vec3(x, (scanPoint.y - 1).toDouble(), z).toVec3i()))){
            ...
            if (origin == destination){
                ...
                if (target is ServerPlayer){
                    ...
                    if (floorNeeded && !isFlying){
                        destination.setBlockAndUpdate((floorSpot), HexBlocks.CONJURED_BLOCK.get().defaultBlockState())
                        BlockConjured.setColor(destination, floorSpot, colorizer)
                    }
                } else {
                    target = target.changeDimension(
                        DimensionTransition(
                            destination,
                            Vec3(x, y, z),
