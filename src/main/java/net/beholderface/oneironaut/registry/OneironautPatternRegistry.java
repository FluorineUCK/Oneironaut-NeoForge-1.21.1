...
public class OneironautPatternRegistry {
    ...
    public static HexPattern GETDIM_OVERWORLD = register(HexPattern.fromAngles("wqwqwqwqwqwawedewdwedew", HexDir.NORTH_EAST), "getdim/overworld", new OpSpecificDim(ResourceLocation.parse("minecraft:overworld"), false, ()->{return OneironautConfig.getServer().getAllowOverworldReflection();}));
    public static HexPattern GETDIM_NETHER = register(HexPattern.fromAngles("wqwqwqwqwqwaqaaqaw", HexDir.NORTH_EAST), "getdim/nether", new OpSpecificDim(ResourceLocation.parse("minecraft:the_nether"), true, ()->{return OneironautConfig.getServer().getAllowNetherReflection();}));
    ...
    /**
     * Registers actions during NeoForge's custom-registry event. Registering
     * directly against HexActions.REGISTRY from the mod constructor forces
     * HexItems to initialize after the vanilla item registry has frozen.
     */
    public static void register(RegisterEvent event) {
        ...
        for (Triple<HexPattern, ResourceLocation, Action> patternTriple : PATTERNS) {
            event.register(HexRegistries.ACTION, patternTriple.getSecond(), () ->
                    new ActionRegistryEntry(patternTriple.getFirst(), patternTriple.getThird()));
        }
        ...
        event.register(HexRegistries.ACTION, Oneironaut.id("craftrod"), () ->
                new ActionRegistryEntry(
                        HexPattern.fromAngles("eqqqqqawweqqqqqawweqqqqqawwdeqewwwwweqeeeqewwwwweqe", HexDir.EAST),
                        new OpMakePackagedSpell(OneironautItemRegistry.REVERBERATION_ROD.get(), MediaConstants.CRYSTAL_UNIT * 10)));
        event.register(HexRegistries.ACTION, Oneironaut.id("craftbottomlesstrinket"), () ->
                new ActionRegistryEntry(
                        HexPattern.fromAngles("wwqeeeeewqqqqqewwaqeqwqeqqqeqwqeq", HexDir.EAST),
