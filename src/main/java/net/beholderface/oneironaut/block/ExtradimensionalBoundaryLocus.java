...
public class ExtradimensionalBoundaryLocus extends BlockCircleComponent {
    ...
    public ExtradimensionalBoundaryLocus(net.minecraft.world.level.block.state.BlockBehaviour.Properties settings) {
        super(settings);
        this.registerDefaultState(this.stateDefinition.any().setValue(BlockCircleComponent.ENERGIZED, false));
    }
    ...
    public ControlFlow acceptControlFlow(CastingImage imageIn, CircleCastEnv env, Direction enterDir, BlockPos pos, BlockState bs, ServerLevel world) {
        CompoundTag nbt = imageIn.getUserData();
        Set<BlockPos> visited = toPositionSet(nbt.getList(TAG_BOUNDARY_LIST, Tag.TAG_INT_ARRAY));
        visited.add(pos);
    ...
    public static Set<BlockPos> toPositionSet(ListTag list) {
        ...
        for (Tag element : list){
            if (element instanceof net.minecraft.nbt.IntArrayTag intArray && intArray.getAsIntArray().length >= 3) {
                int[] values = intArray.getAsIntArray();
                output.add(new BlockPos(values[0], values[1], values[2]));
            }
        }
        return output;
    }
