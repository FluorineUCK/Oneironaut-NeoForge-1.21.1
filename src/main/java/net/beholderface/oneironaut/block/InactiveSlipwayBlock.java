...
public class InactiveSlipwayBlock extends Block {
    public InactiveSlipwayBlock(net.minecraft.world.level.block.state.BlockBehaviour.Properties settings) {
        super(settings);
    }
    ...
    public static void init(){
        ...
        for (int i = 0; i < 32; i++){
            for(var pigmentSupplier : HexItems.DYE_PIGMENTS.values()){
                ItemDyePigment pigment = pigmentSupplier.get();
    ...
    public void animateTick(BlockState state, Level world, BlockPos pos, RandomSource random) {
        if (colors != null){
            ...
            for(var ignored : HexItems.DYE_PIGMENTS.values()){
